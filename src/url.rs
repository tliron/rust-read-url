...
impl Context {
    ...
    ///
    /// To support relative URLs, see [Context::new_valid_url].
    ///
    /// If you are expecting either a URL or a file path, consider [Context::new_any_or_file_url].
    ...
    ///
    /// In essence attempts to parse the URL via [Context::new_url] and if that fails
    /// treats the URL as a file path and returns a [FileUrl].
    ///
    /// To support relative URLs, see [Context::new_valid_any_or_file_url].
    ///
    ...
    /// scheme.
    pub fn new_any_or_file_url(self: &ContextRef, url: &str) -> Result<UrlRef, UrlError> {
        match self.clone().new_url(url) {
            Ok(url) => Ok(url),

            Err(_) => {
                let url = self.new_file_url(Path::new(url));
                Ok(url.into())
            }
        }
    }
    ...
    /// possible to call [URL::open] on it. Of course this can't guarantee that
    /// future calls to [URL::open] will succeed.
    ///
    ...
    /// [Context::new_valid_any_or_file_url].
