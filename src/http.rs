...
impl Context {
    ...
    pub fn new_http_url_from_str(self: &ContextRef, url: &str) -> Result<HttpUrl, UrlError> {
        let url = url::Url::parse(url)?;
        Ok(self.new_http_url(url))
    }

    pub fn new_valid_http_url_from_str(self: &ContextRef, url: &str) -> Result<HttpUrl, UrlError> {
        let url = url::Url::parse(url)?;
